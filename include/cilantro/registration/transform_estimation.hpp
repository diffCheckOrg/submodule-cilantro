...
estimateTransformPointToPointMetric(
  ...
  Eigen::Matrix<ScalarT, TransformT::Dim, Eigen::Dynamic, Eigen::RowMajor> dst_centered(
      dst.colwise() - mu_dst);
  Eigen::Matrix<ScalarT, TransformT::Dim, Eigen::Dynamic, Eigen::RowMajor> src_centered(
      src.colwise() - mu_src);

  Eigen::Matrix<ScalarT, TransformT::Dim, TransformT::Dim> sigma(
      (ScalarT(1) / ScalarT(dst.cols())) * (dst_centered * src_centered.transpose()));

  Eigen::JacobiSVD<Eigen::Matrix<ScalarT, TransformT::Dim, TransformT::Dim>> svd(
...
estimateTransformPointToPointMetric(
  ...
  Eigen::Matrix<ScalarT, NumUnknowns, NumUnknowns> AtA(
      Eigen::Matrix<ScalarT, NumUnknowns, NumUnknowns>::Zero());
...
#pragma omp parallel MATRIX_SUM_REDUCTION(ScalarT, NumUnknowns, NumUnknowns, AtA) \
    MATRIX_SUM_REDUCTION(ScalarT, NumUnknowns, 1, Atb)
#endif
  {
...
#pragma omp for nowait
  ...
  tform.linear() =
      Eigen::Map<Eigen::Matrix<ScalarT, Dim, Dim, Eigen::RowMajor>>(theta.data(), Dim, Dim);
...
bool estimateTransformPointToPointMetric(
    const ConstVectorSetMatrixMap<typename TransformT::Scalar, TransformT::Dim>& dst,
    const ConstVectorSetMatrixMap<typename TransformT::Scalar, TransformT::Dim>& src,
    const CorrSetT& corr, TransformT& tform) {
  VectorSet<typename TransformT::Scalar, TransformT::Dim> dst_corr, src_corr;
  selectCorrespondingPoints<typename TransformT::Scalar, TransformT::Dim, CorrSetT>(
      corr, dst, src, dst_corr, src_corr);
  return estimateTransformPointToPointMetric(dst_corr, src_corr, tform);
}

// Rigid, combined metric, 2D, iterative
template <class TransformT, class PointCorrSetT, class PlaneCorrSetT,
...
typename std::enable_if<int(TransformT::Mode) == int(Eigen::Isometry) && TransformT::Dim == 2,
                        bool>::type
estimateTransformCombinedMetric(
    ...
    typename TransformT::Scalar point_to_plane_weight, TransformT& tform, size_t max_iter = 1,
    ...
    const Vector<typename TransformT::Scalar, 2>& dst_mean =
        Vector<typename TransformT::Scalar, 2>::Zero(),
    const Vector<typename TransformT::Scalar, 2>& src_mean =
        Vector<typename TransformT::Scalar, 2>::Zero()) {
  typedef typename TransformT::Scalar ScalarT;
...
#pragma omp parallel MATRIX_SUM_REDUCTION(ScalarT, 3, 3, AtA) \
    MATRIX_SUM_REDUCTION(ScalarT, 3, 1, Atb)
#endif
    {
...
#pragma omp for nowait
        ...
        for (size_t i = 0; i < point_to_point_correspondences.size(); i++) {
          ...
          const ScalarT weight =
              point_to_point_weight *
              point_corr_evaluator(corr.indexInFirst, corr.indexInSecond, corr.value);
...
#pragma omp for nowait
        ...
        for (size_t i = 0; i < point_to_plane_correspondences.size(); i++) {
          ...
          const ScalarT weight =
              point_to_plane_weight *
              plane_corr_evaluator(corr.indexInFirst, corr.indexInSecond, corr.value);
          const Vector<ScalarT, 2> d = dst_p.col(corr.indexInFirst) - dst_mean;
...
estimateTransformCombinedMetric(
    ...
    const PointCorrWeightEvaluatorT& point_corr_evaluator = PointCorrWeightEvaluatorT(),
    const PlaneCorrWeightEvaluatorT& plane_corr_evaluator = PlaneCorrWeightEvaluatorT(),
    const Vector<typename TransformT::Scalar, 3>& dst_mean =
        Vector<typename TransformT::Scalar, 3>::Zero(),
    const Vector<typename TransformT::Scalar, 3>& src_mean =
        Vector<typename TransformT::Scalar, 3>::Zero()) {
  typedef typename TransformT::Scalar ScalarT;
...
#pragma omp parallel MATRIX_SUM_REDUCTION(ScalarT, 6, 6, AtA) \
    MATRIX_SUM_REDUCTION(ScalarT, 6, 1, Atb)
#endif
    {
      if (has_point_to_point_terms) {
...
estimateTransformCombinedMetric(
    ...
    const PointCorrWeightEvaluatorT& point_corr_evaluator = PointCorrWeightEvaluatorT(),
    const PlaneCorrWeightEvaluatorT& plane_corr_evaluator = PlaneCorrWeightEvaluatorT(),
    const Vector<typename TransformT::Scalar, TransformT::Dim>& dst_mean =
        Vector<typename TransformT::Scalar, TransformT::Dim>::Zero(),
    const Vector<typename TransformT::Scalar, TransformT::Dim>& src_mean =
        Vector<typename TransformT::Scalar, TransformT::Dim>::Zero()) {
  typedef typename TransformT::Scalar ScalarT;
  enum { Dim = TransformT::Dim, NumUnknowns = TransformT::Dim * (TransformT::Dim + 1) };
...
#pragma omp for nowait
  ...
  return has_point_to_point_terms * point_to_point_correspondences.size() +
...
// Szymon Rusinkiewicz. A Symmetric Objective Function for ICP. ACM Transactions on Graphics (Proc.
// SIGGRAPH) 38(4), July 2019.
template <class TransformT, class PointCorrSetT, class PlaneCorrSetT,
          class PointCorrWeightEvaluatorT =
              UnityWeightEvaluator<typename TransformT::Scalar, typename TransformT::Scalar>,
          class PlaneCorrWeightEvaluatorT =
...
typename std::enable_if<int(TransformT::Mode) == int(Eigen::Isometry) && TransformT::Dim == 2,
                        bool>::type
estimateTransformSymmetricMetric(
    const ConstVectorSetMatrixMap<typename TransformT::Scalar, 2>& dst_p,
    const ConstVectorSetMatrixMap<typename TransformT::Scalar, 2>& dst_n,
    const ConstVectorSetMatrixMap<typename TransformT::Scalar, 2>& src_p,
...
#pragma omp for nowait
        ...
        for (size_t i = 0; i < point_to_plane_correspondences.size(); i++) {
          ...
          const ScalarT weight =
              ...
              plane_corr_evaluator(corr.indexInFirst, corr.indexInSecond, corr.value);
          const Vector<ScalarT, 2> d = dst_p.col(corr.indexInFirst) - dst_mean;
          const Vector<ScalarT, 2> n =
              dst_n.col(corr.indexInFirst) + tform.linear() * src_n.col(corr.indexInSecond);
          const Vector<ScalarT, 2> s = tform * (src_p.col(corr.indexInSecond) - src_mean);
